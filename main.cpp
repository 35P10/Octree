...
int main()
{
    ...
    // Inserta varios puntos en el octree
    ...
    // octree.insert(Point(10, 11, 12));

    // Inserta más puntos
     octree.insert(Point(3, 6, 9));
     octree.insert(Point(2, 4, 8));
     octree.insert(Point(11, 10, 9));
     octree.insert(Point(5, 2, 7));
    // Agrega más inserciones según sea necesario

    // Imprime los nodos hoja
    std::cout << "Leaf Nodes:\n";
	
    GL_Cube a(glm::vec3(1,1,1), glm::vec3(1,1,1));
    GL_Cube b = a;
    GL_Cube c;

    b.translate(glm::vec3(0.0f, 1.0f, 0.0f));
	// render loop
	// -----------
        glm::mat4 aaaa = glm::mat4(1.0f);
        unsigned int i = 0;
